...
contract FunctionsConsumerExample is FunctionsClient, ConfirmedOwner {
    ...
    // now the easy part
    function parseResponse() public pure returns (
        bool finished,
        bool away_win,
        uint16 margin,
        uint32 game_id,
        ...
        bytes memory input = bytes("0x000000000000000000000000000000000000000000000000000000007784003b");
        (,,,bits) = abi.decode(input, (uint256, uint256, uint256, uint256));
        finished = bits & 1 == 1;
        away_win = (bits >> 1) & 1 == 1;
